...
big_tiles_epsg4326_lat = []
big_tiles_epsg4326_long = []
big_tiles_epsg25832_lat = []
big_tiles_epsg25832_long = []

for i in range(len(df_points)):
    lat = df_points.iloc[i,0]
    long = df_points.iloc[i,1]
    number_of_restaurants = get_number_of_restaurants(lat, long, radius)
    if number_of_restaurants > 950:
        big_tiles_epsg4326_lat.append(lat)
        big_tiles_epsg4326_long.append(long)
        big_tiles_epsg25832_lat.append(etrs_latitudes[i])
        big_tiles_epsg25832_long.append(etrs_longitudes[i])

df_big_tiles = pd.DataFrame({'epsg4326_lat': big_tiles_epsg4326_lat,
                             'big_tiles_epsg4326_long': big_tiles_epsg4326_long,
                             'big_tiles_epsg25832_lat': big_tiles_epsg25832_lat,
                             'big_tiles_epsg25832_long': big_tiles_epsg25832_long})
    
df_big_tiles.to_csv('big_tiles.csv')
